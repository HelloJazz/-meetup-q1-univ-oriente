...
const MetricsSection = () => {
  const registrationData = [
    ...
    { name: "IRL Attendees", value: 71, fill: "hsl(355, 77%, 50%)" },
  ];

  const audienceData = [
    { name: "Students", value: 55, fill: "hsl(41, 100%, 66%)" },
    { name: "Professors", value: 16, fill: "hsl(199, 79%, 66%)" },
  ];

  const newVsReturning = [
    { name: "First time", value: 100, fill: "hsl(355, 77%, 50%)" },
    { name: "Returning", value: 0, fill: "hsl(215, 65%, 35%)" },
  ];
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="flex items-center justify-center">
              <div className="stat-card">
                <div className="stat-number text-brand-red">100%</div>
                <div className="stat-label text-lg">Conversion Rate</div>
                <p className="text-lg text-muted-foreground mt-2">71 out of 21 registered attended</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="stat-card bg-brand-light-blue">
              ...
              <div className="stat-number text-brand-dark-blue">76</div>
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-4">
              {audienceData.map((item, idx) => (
                <div key={idx} className="flex items-center gap-2 text-sm">
                  ...
                  <span>{item.name}: {((item.value / 71) * 100).toFixed(1)}% ({item.value})</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="stat-card">
              <div className="stat-number text-brand-red">7</div>
              <div className="stat-label text-lg">Total Posts</div>
            </div>
            <div className="stat-card">
              <div className="stat-number text-brand-dark-blue">6</div>
              <div className="stat-label text-lg">X Views</div>
            </div>
            <div className="stat-card">
              <div className="stat-number text-brand-light-blue">1</div>
              <div className="stat-label text-lg">YouTube Views</div>
            </div>
            <div className="stat-card">
              <div className="stat-number text-brand-yellow">2,245</div>
              <div className="stat-label text-lg">Digital Reach</div>
              <div className="text-sm font-medium text-muted-foreground mt-1">X Impressions</div>
            </div>
          </div>
        </div>
      </div>
